...
本文档用于本地演示“投标 Markdown 输入 → 切章目录 → 可编辑目录 → 逐章生成 → 版本管理 → 合并 final.md”的工作台流程。
...
- `.coalplan-data/artifacts/{project_id}`：每个项目的输入、切章、prompt trace、章节、final.md
...
- `COALPLAN_DEEPSEEK_MODEL=deepseek-v4-pro`：用于具体生成。
...
```text
C:\Users\Lenovo\Documents\煤火\coalplan-ai\data\real_inputs\ningxia_coal_fire_bid.normalized.md
```
...
- `.coalplan-data/artifacts/{project_id}/final.md`：最终合并 Markdown。
...
确认后端是在设置环境变量之后启动的。环境变量改动不会自动影响已经运行的后端，需要重启 uvicorn。
